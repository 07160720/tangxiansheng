...
KindEditor.plugin('filemanager', function(K) {
	...
	self.plugin.filemanagerDialog = function(options) {
		...
		function createView(result) {
			...
			/*给删除键添加事件，利用ajax传送给后台文件*/
			
			var del=document.getElementsByClassName ("xl_span");//这里的xl_span对应着刚才插入的删除按钮上的class
			for(var i=0;i<del.length;i++){
				...
				del[i].onclick=function(){
				    ...
				    var this_span = K(this);
				    K.ajax("../js/kindeditor/php/file_manager_json.php?action=delete&&url="+url, function(){//kajax的回调函数
				    	this_span.parent().remove();//移除窗口上的被删图片 ，实现刷新
				    });
//				    var ajax=null;  
...
//				    }
//				    
//				    ajax.open("GET","../js/kindeditor/php/file_manager_json.php?action=delete&&url="+url,true);
//				    ajax.onreadystatechange=K(this).parent().remove();  //移除窗口上的被删图片 ，实现刷新
//				    ajax.send(null);
