...
<div id="pagehead" style="width: 100%;position: fixed; top: 0px;background:#DDEEF2;padding: 12px;border-bottom: 2px solid #BBDDE5; ">
	...
	<span style="font-size: 13px;color: gray;" >修改栏目</span>
</div>
<!--当前操作  -->
<!-- start client form -->
<div class="tab-div">
    ...
    <div id="tabbody-div">
      <form enctype="multipart/form-data" action="column.php" method="post" name="theForm" onsubmit="return validate()" >
    	...
    	<?php
            ...
            include '../includes/db_connect.php'; $db = new MyDB();
            $data = $db->GetData($sql);
        ...
        ?>
          ...
          <tr>
            ...
            <td>
            	<select name="model">
                <?php 
            	   echo '<option value="'.$data['model'].'">'.$cfg_model[$data['model']].'</option>';
            	   foreach ($cfg_model as $name => $value){
            	       if ($data['model'] == $name) continue;
          ...
          <tr>
            ...
            <td>
            	<input type="text" name="pid" value="<?php echo $data["pid"]; ?>" size="1" onblur="isInteger(this)"/> 
          ...
          <tr >
            ...
            <td >
            	<input type="text" name="catalog" value="<?php echo $data["catalog"]; ?>" size="30"/>
            	<span class="link-span">请不要使用中文名称的目录名</span>
            </td>
          </tr>
          <tr>
            <td class="label">默认链接:</td>
            <td><input type="text" name="link" value="<?php echo $data["link"]; ?>"  size="80" /></td>
          </tr>
          <tr>
            <td class="label">打开方式:</td>
            <td>
            	<input type="radio" name="target" value="_blank" <?php echo @$data[target]!=""?'checked="checked"':'';?>"/>空白页&nbsp;
                <input type="radio" name="target" value="" <?php echo @$data[target]==""?'checked="checked"':'';?>"/>当前页 
            </td>
          </tr>
          <tr>
            <td class="label">排列顺序:</td>
            <td>
            	<input type="text" name="porder" value="<?php echo $data["porder"]; ?>" size="1"  onblur="isInteger(this)"/> 
          ...
          <tr>
            ...
            <td>
            	<input type="text" name="template" value="<?php echo $data["template"]; ?>" size="20" onblur="getFileSize(this)"/> 
            </td>
          </tr>
           <tr>
            <td class="label">是否显示栏目:</td>
            <td>
            	<input type="radio" name="display" value="show" <?php echo $data['display']!="hide"?'checked="checked"':'';?>/>显示&nbsp;
                <input type="radio" name="display" value="hide" <?php echo $data['display']=="hide"?'checked="checked"':'';?>/>隐藏 
            </td>
          </tr>
          <tr>
             <td >&nbsp;</td>
