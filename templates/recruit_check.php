...
<div id="pagehead" style="width: 100%;position: fixed; top: 0px;background:#DDEEF2;padding: 12px;border-bottom: 2px solid #BBDDE5; ">
	...
	<span style="font-size: 13px;color: gray;" >管理招聘</span>
</div>
<!--当前操作  -->

<form enctype="multipart/form-data" name="listForm" id="list-form" action="recruit.php" method="post"   onsubmit="return deleteData('all')">
	<div  id="list-div">
...
<?php 
   ...
   if ($sum == 0) {
    ...
    $sql = "SELECT * FROM `recruit` ORDER BY `date` DESC LIMIT " . $from . "," . $item;
    ...
    while(!! $data = mysql_fetch_array($resource)){
        ...
        <tr class="hover">
        	...
        	<td onclick="checkboxes'.$i.'.click()"> <span>'.$data['position'] .'</span></td>
        	<td align="center"><span>'.$data['quantity'] .'</span></td>
        	<td align="center"><span>'.$data['place'] .'</span></td>
        	<td align="center"><span>'.$data['date'] .'</span></td>
            <td align="center" >
                
            	<a href="recruit_modify.php?id='.$data['id'].'" title="编辑"><img src="../images/icon_edit.gif" width="16" height="16" border="0" /></a>
            	
            	<a href="javascript:deleteData('.$data['id'].');" id="delete'.$data['id'].'" data="'.$data['position'].'" title="删除">
                <img src="../images/icon_trash.gif" width="14" height="15" border="0" alt="删除"/></a>
...
function deleteData(id){
	if(id == 'all'){
		...
		if(confirm("确定要删除 '"+title+"' 吗？")){
			window.location.href="recruit.php?act=delete&id="+id;
			return true;
		}else
			return false;
	}
